...
import { updateAnimalSchema } from '@/server/schemas/animal';
import api from '@/api';
...
type UpdateAnimalValues = z.infer<typeof updateAnimalSchema>;

export type UpdateAnimalProps = {
  animal: AnimalDetailsDTO;
};

const UpdateAnimal: React.FC<UpdateAnimalProps> = ({ animal }) => {
  const router = useRouter();
  const { toast } = useToast();

  const form = useForm<UpdateAnimalValues>({
    mode: 'all',
  ...
  const { mutate: updateAnimal, isPending } = api.animal.updateAnimal.useMutation({
    onError: error => {
      toast({
        title: 'Помилка!',
    ...
    onSuccess: async data => {
      toast({
        title: 'Успішно!',
        description: 'Інформацію про тваринку оновлено',
      });
  ...
  const handleUpdateAnimal = (values: UpdateAnimalValues): void => {
    updateAnimal(values);
  };

  return (
    <div className=" flex w-full flex-col items-center gap-4">
      <h1 className="font-h1 mb-4 text-orange-900">Оновити інформацію про {animal.name}</h1>
      <h2 className="font-h2 text-orange-700">Тут Ви можете оновити інформацію про тваринку</h2>

      <AppForm {...form}>
        <form className="mt-8 flex w-full flex-col gap-6" onSubmit={form.handleSubmit(handleUpdateAnimal)}>
            ...
            <div className="flex grow-[3] flex-col gap-4 overflow-hidden rounded-lg bg-orange-100 p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormSelect //
                  ...
                  name="status"
                  control={form.control}
                  options={getOptions(AnimalStatusEnum)}
                />
              </div>
